...
Vercel에서 GitHub 저장소를 Import할 때 Root Directory는 저장소 루트로 둡니다. 프로젝트가 GitHub에 연결되면 main 브랜치 push마다 루트 `dist`가 자동으로 빌드되어 배포됩니다. Vercel에서 생성된 Production Domain이 README 상단의 미리보기 링크와 다르면 해당 링크 URL만 실제 도메인으로 바꿉니다.
