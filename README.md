...
- Build Command: `cd .. && pnpm --filter @ncai/design-system-preview build`
- Output Directory: `dist`

Vercel에서 GitHub 저장소를 Import할 때 Root Directory는 `preview`로 둡니다. `preview/vercel.json`은 monorepo 루트에서 의존성을 설치하고 preview 앱을 빌드한 뒤, Vercel 프로젝트 루트 기준 `dist`를 배포하도록 설정합니다. Vercel에서 생성된 Production Domain이 README 상단의 미리보기 링크와 다르면 해당 링크 URL만 실제 도메인으로 바꿉니다.
